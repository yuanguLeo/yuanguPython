...
'''
UTC(世界协调时间)：格林尼治天文时间，世界标准时间，在中国来说是UTF+8
DST(夏令时)：是一种节约能源而人为规定时间制度，在夏季调快一个小时

时间的表示形式：
1、时间戳
以整型或浮点型表示时间的一个以秒为单位的时间间隔，
这个时间间隔的基础值从1970年1月1日零点开始算起
...
%P 本地am或pm的相应符
%S 秒（01-60）
%U 一年中的星期数。（00-53星期天是一个星期的开始）第一个星期天之前的所有天数都在第0周
...
#将时间戳转为字符串  time.asctim(time.localtime(time.time()))
str = time.ctime(c)
print(str)

#将时间元组转为给定格式的字符串，参数2为时间元组，如果没有参数2，默认转当前时间
